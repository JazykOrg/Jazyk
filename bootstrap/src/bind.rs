...
use crate::gen::{artifact_path, hash_file, hash_files, GenSettings, Ledger, ReqRow, RowHashes, TestRef};
...
pub fn pending(store: &Store, gs: &GenSettings) -> Vec<Value> {
    ...
    for (rid, r) in &store.graph.requirements {
        ...
        let entity = r.entities.first().map(|e| store.resolve_id(e).to_string()).unwrap_or_default();
        out.push(json!({
...
pub fn task(store: &Store, rid: &str, gs: &GenSettings) -> Result<Value, String> {
    ...
    let reason = ledger
        .requirements
        .get(&rid)
        .map(|row| if hash_hex(&r.ears) != row.hashes.requirement { "requirement-changed" } else { "artifact-gone" })
        .unwrap_or("unbound");
    Ok(json!({
        "requirement": rid,
        "entity": entity,
        ...
        "suggestedTestName": crate::gen::test_name(&rid, &r.ears),
...
pub fn record(
    ...
    if kind == "programmatic" && run.is_empty() {
        return Err("a programmatic test needs test.run: the command whose exit code is the verdict".into());
    }
    let tref = TestRef {
        kind: kind.into(),
    ...
    if !path.exists() {
        return Err(format!("test artifact `{}` does not exist; write it before recording", tref.artifact));
    }
    if kind == "programmatic" {
        let content = std::fs::read_to_string(&path).unwrap_or_default();
        if !content.contains(&name) {
            return Err(format!("test artifact `{}` does not contain the test name `{}`", tref.artifact, name));
        }
    }
    ...
    let mut files: Vec<String> = files.iter().map(|f| f.trim().to_string()).filter(|f| !f.is_empty()).collect();
    files.sort();
    files.dedup();
    for f in &files {
    ...
    let ledger = Ledger::load(&store.out);
    let (status, reason) = crate::verify::status_of(store, &rid, ledger.requirements.get(&rid).unwrap(), gs);
    Ok(json!({
        "recorded": rid,
        "verdict": verdict,
        "status": status,
...
fn collect_code_files(dir: &Path, out_dir: &Path, out: &mut Vec<std::path::PathBuf>) {
    ...
    for e in entries.flatten() {
        ...
        if p == out_dir || name.starts_with('.') || name.starts_with("jazyk-out") || name == "jazyk.toml" {
            continue;
        }
        if p.is_dir() {
...
pub fn run_all(
    ...
    for t in &owed {
        ...
        trace.line("bind", &format!("{} ({})", rid, t["reason"].as_str().unwrap_or("")));
        ...
        let ledger = Ledger::load(&store.out);
        let live = store.graph.requirements.get(&rid).map(|r| hash_hex(&r.ears)).unwrap_or_default();
        match ledger.requirements.get(&rid) {
            Some(row) if row.hashes.requirement == live => bound += 1,
            Some(_) => {
                trace.line("bind", &format!("{} recorded a stale statement hash; still owed", rid));
                failures += 1;
            }
            None => {
                trace.line("bind", &format!("{} ended without record_binding; still owed", rid));
                failures += 1;
            }
        }
    }
...
mod tests {
    ...
    fn tmp_store() -> Store {
        ...
        let out = std::env::temp_dir().join(format!("jazyk-bind-test-{}-{}", std::process::id(), n));
        std::fs::remove_dir_all(&out).ok();
        std::fs::create_dir_all(&out).unwrap();
        let mut s = Store { out, ..Default::default() };
        s.graph.entities.insert(
            "ent:shop".into(),
            crate::model::Entity { name: "Shop".into(), ..Default::default() },
        );
        s.graph.requirements.insert(
            "req:shop-1".into(),
            crate::model::Requirement {
                ears: "The shop shall list items.".into(),
    ...
    fn an_unbound_requirement_is_bind_work() {
        let s = tmp_store();
        let gs = GenSettings { deliverable: s.out.join("product"), worker: "agentic".into(), code: Vec::new() };
        let p = pending(&s, &gs);
        assert_eq!(p.len(), 1);
    ...
    fn recording_a_failing_bind_with_no_files_reads_unimplemented() {
        ...
        let v = record(&s, "req:shop-1", &[], &test, "fail", Some("no implementation found"), &gs).unwrap();
    ...
    fn recording_a_passing_bind_with_files_reads_verified() {
        ...
        std::fs::write(gs.deliverable.join("tests/shop.sh"), format!("# {}\nexit 0\n", name)).unwrap();
        let test = json!({"kind": "programmatic", "artifact": "tests/shop.sh", "name": name, "run": "sh tests/shop.sh"});
        let v = record(&s, "req:shop-1", &["shop.sh".into()], &test, "pass", None, &gs).unwrap();
        ...
        assert!(!un.contains(&"shop.sh".to_string()), "claimed file listed unclaimed: {:?}", un);
    }
