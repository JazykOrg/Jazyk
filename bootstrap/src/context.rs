// The context engine: the loaded set, an explicit working set of graph nodes,
// bounded by a budget, with a rendered status the model reads on every round.
// Owned by a session. Mirrors docs/compiler/context.md.
...
// The context of one ledger package (bind, generate, verify): the target rendered
// through the loaded set, closed by the status block with its open handles.
// Mirrors docs/consumers/bind.md#the-bind-goal and docs/consumers/gen.md.
pub fn ledger_context(store: &Store, target: &str) -> String {
    let mut set = LoadedSet::new(crate::limits::CONTEXT_BUDGET);
    match set.load(store, target, 1) {
        Ok(text) => format!("{}\n{}", text, set.render_status("", 0, &BTreeSet::new())),
        Err(_) => String::new(),
    }
}
