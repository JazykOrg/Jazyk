...
fn checks(
            ...
            for i in 0..list.len() {
                for j in i + 1..list.len() {
                    ...
                    if sa.doc == sb.doc {
                        ...
                        // ("has an id" / "has a name"), not duplicates.
    ...
    for (rid, r) in &store.graph.requirements {
        ...
        if !store.quote_locates(&src.doc, &src.section, &src.quote) {
            f.push((
    ...
    for p in parked {
        f.push((
            ...
            "warning".into(),
            format!(
                "work item {} {} was parked; the next build resumes it",
                p.task, p.target
            ),
        ));
    }
...
pub fn compile(proj: &Project, llm: &Llm, out: &Path, trace: &Trace) -> BuildReport {
    ...
    let store = Mutex::new(Store::open_for_build(out));
    let gs = crate::gen::GenSettings::resolve(proj);
    let _ = &gs;
    let (parsed, links) = parse_all(proj);
    // Resume parked work from the previous build first.
    let previously_parked: Vec<WorkItem> = store.lock().unwrap().status.parked_items();
    ...
    let align_items: Vec<WorkItem> = {
        ...
        let mut items: Vec<WorkItem> = previously_parked
            .iter()
            .filter(|p| p.task == "align-doc")
            .cloned()
            .collect();
    ...
    let budget_cap = crate::limits::BUILD_SESSION_FACTOR
        * (total_dirty + align_items.len() + previously_parked.len()).max(1)
        + crate::limits::BUILD_SESSION_FLOOR;
    ...
    let (dirty, levels) = if align_items.is_empty() {
        ...
        if trace.is_cancelled() {
            ...
            let (applied, touched, changed, parked) =
                run_wave(&store, &runner, &align_items, &parsed, out, trace);
    ...
    for mut level_items in wave1 {
        ...
        let (blocked, runnable): (Vec<WorkItem>, Vec<WorkItem>) = level_items
            .drain(..)
            .partition(|w| aligning_parked.contains(&w.target));
        ...
        let (applied, touched, changed, parked) =
            run_wave(&store, &runner, &level_items, &parsed, out, trace);
        applied_total += applied;
        touched_all.extend(touched);
        changed_all.extend(changed);
        parked_all.extend(parked);
    ...
    let fixup: Vec<WorkItem> = {
        let s = store.lock().unwrap();
        let parked_docs: BTreeSet<&String> = parked_all
            .iter()
            .filter(|p| matches!(p.task.as_str(), "reconcile-doc" | "align-doc"))
            .map(|p| &p.target)
            .collect();
        s.docs
            .iter()
    ...
    if !fixup.is_empty() {
        if (turns as usize) >= budget_cap || trace.is_cancelled() {
            ...
            trace.line(
                "reconcile",
                &format!(
                    "fix-up pass: {} document(s) with uncovered sections or stale anchors",
                    fixup.len()
                ),
            );
            turns += fixup.len() as u32;
            let (applied, touched, changed, parked) =
                run_wave(&store, &runner, &fixup, &parsed, out, trace);
            applied_total += applied;
            touched_all.extend(touched);
            changed_all.extend(changed);
            parked_all.extend(parked);
    ...
    // Reviews owed from earlier builds or other consumers: the change records are the
    // durable form of changed_all (docs/compiler/graph.md#change-records).
    pair_targets.extend(
        store
            .lock()
            .unwrap()
    ...
    if !pair_items.is_empty() {
        if (turns as usize) + pair_items.len() > budget_cap || trace.is_cancelled() {
            ...
            trace.line(
                "reconcile",
                &format!("pair review: {} changed requirement(s)", pair_items.len()),
            );
            turns += pair_items.len() as u32;
            let (applied, touched, _changed, parked) =
                run_wave(&store, &runner, &pair_items, &parsed, out, trace);
            applied_total += applied;
            touched_all.extend(touched);
...
pub fn finalize(
    ...
    let unprocessed = s
        ...
        .count();
    // A change record whose subject no longer exists is cleared, and so is a pair
    // review with nothing tying it to a judgment: no computed neighbors and no open
    // judged diagnostic naming it.
    let stale_reqs: Vec<String> = s
        .status
        .changed_subjects(&crate::store::REQ_REVIEW_KINDS)
        .into_iter()
        .filter(|r| r.starts_with("req:") && !s.pair_review_due(r))
        .collect();
    for r in &stale_reqs {
        s.status.clear_changes(&crate::store::REQ_REVIEW_KINDS, r);
    }
    // A section reconciled since its record was written (a coverage mark claimed at a
    // later generation) and a stale anchor whose quotes locate again owe nothing.
    let claimed_after = |claimed: &Option<String>, generation: u64| {
        claimed
            .as_deref()
    ...
    let section_covered = |c: &crate::model::ChangeRecord| {
        crate::model::split_section_ref(&c.subject).is_some_and(|(doc, sec)| {
            s.docs
                .get(&doc)
                .and_then(|r| r.coverage.get(&sec))
                .is_some_and(|cov| claimed_after(&cov.claimed_by, c.generation))
        })
    };
    let anchors_settled = |c: &crate::model::ChangeRecord| {
        c.detail["anchors"]
            .as_array()
            .map(|a| {
                a.iter().filter_map(|v| v.as_str()).all(|id| {
                    !s.status.reevaluate.iter().any(|x| x == id)
                        && match s.graph.requirements.get(id) {
                            Some(r) => r
                                .source
                                .as_ref()
                                .map(|q| s.quote_locates(&q.doc, &q.section, &q.quote))
                                .unwrap_or(true),
                            None => !s.graph.entities.contains_key(id),
                        }
                })
            })
            .unwrap_or(true)
    ...
    let keep: Vec<bool> = s
        ...
        .map(|c| match c.kind.as_str() {
            crate::store::CHANGE_SECTION_DIRTY => !section_covered(c),
            crate::store::CHANGE_ANCHOR_STALE => !anchors_settled(c),
            _ => true,
        })
        .collect();
    let mut i = 0;
    s.status.changes.retain(|_| {
        let k = keep[i];
        i += 1;
        k
    });
    s.status
        .reevaluate
        .retain(|r| s.graph.requirements.contains_key(r));
    s.status.parked = parked_all
        .iter()
        .map(|p| p.to_goal(GoalState::Parked))
        .collect();
    let open_reviews = s
        .status
        .changed_subjects(&crate::store::ENTITY_REVIEW_KINDS)
        .iter()
        .filter(|e| s.graph.entities.contains_key(*e))
        .count()
        + s.status
            .changed_subjects(&crate::store::REQ_REVIEW_KINDS)
            .iter()
            .filter(|r| s.graph.requirements.contains_key(*r))
            .count();
    let open = parked_all.len() as u64 + unprocessed as u64 + open_reviews as u64;
    s.status.verdict = Verdict {
        ...
        open,
        failed: s.status.failed.len() as u64,
        blocked: 0,
        optional: 0,
    };
    ...
    if n > 0 {
        trace.line(
            "reconcile",
            &format!("docsgen: {} requirements document(s)", n),
        );
    }
    ...
    BuildReport {
        verdict: s.status.verdict.to_string(),
        dirty_docs: 0,
        turns: 0,
        applied: 0,
...
mod tests {
    ...
    fn pinned_literals_picks_values_and_skips_words() {
        ...
        assert!(
            !lits.iter().any(|l| l == "username"),
            "a plain word is not pinned: {:?}",
            lits
        );
    }
    ...
    fn drift_check_flags_missing_literals_with_a_question() {
        ...
        assert_eq!(
            f.len(),
            1,
            "only the missing literal fires: {:?}",
    ...
    fn rules_for<'a>(
        f: &'a [(String, String, String, String)],
        rule: &str,
    ...
    fn empty_file_flagged_with_zero_llm_calls() {
        ...
        assert!(hits
            .iter()
            .all(|(_, subj, sev, _)| sev == "warning" && subj != "full.md"));
    }
    ...
    fn duplicate_requirement_splits_warning_and_info() {
        ...
        assert_eq!(hits.len(), 2, "{:?}", hits);
        assert!(hits.iter().any(|(_, subj, sev, msg)| subj == "req:a-1"
            && sev == "warning"
            && msg.contains("keep one")));
        assert!(hits.iter().any(|(_, subj, sev, msg)| subj == "req:b-1"
            && sev == "info"
