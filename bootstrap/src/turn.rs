...
6. Report real problems with report_diagnostic: rule contradiction for requirements that cannot all hold, duplicate-entity for two entities that are one concept, ambiguity for a statement open to more than one reading, missing-link for a concept the documents rely on but never define, lint for spelling or grammar inside a source quote. When the correction is obvious (a misspelling, a wrong plural), attach a prompt with the one suggested edit so the owner applies it in place.
