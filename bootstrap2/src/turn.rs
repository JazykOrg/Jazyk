...
4. Tag each requirement with the entity the statement is about (its own grammatical subject) AND every other entity the statement names: "The user account shall have a username" references both the account and the username field; that reference is what ties the field into the graph. Never substitute a broader system for a named part ("The inventory system manages products" is about the inventory system, not the application containing it). One sentence introduces at most one entity for its subject: "This software is a warehouse management system" defines ONE entity, not two.
